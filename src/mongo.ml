...
let drop_database m =
  wrap_unix send (m,drop_in m (Bson.add_element "dropDatabase" (Bson.create_int32 1l) Bson.empty))

let drop_collection m =
  wrap_unix send (m,drop_in m (Bson.add_element "drop" (Bson.create_string m.collection_name) Bson.empty))
